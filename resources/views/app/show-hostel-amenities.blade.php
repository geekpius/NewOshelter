@if (count($rooms))
<hr class="mt-sm-5">
<h4 class="header-title mt-0 mb-3">Hostel Room Types Amenities</h4>
<hr>
                ...
                <div class="time-item">
                    <div class="item-info">
                        <div class="d-flex justify-content-between align-items-center">
                            <h6 class="m-0">{{ $item->propertyHostelBlock->block_name }} - <small class="text-primary">{{ $item->block_room_type }}</small></h6>
                         </div>
                         
                        <div class="mt-4">
                           @if(count($item->hostelRoomAmenities))
                           @foreach ($item->hostelRoomAmenities as $amenity)
                           <span class="mr-4 font-12 removeAmenity" data-href="{{ route('property.room.amenities.delete', $amenity->id) }}"><span class="fa fa-check-square text-success"></span>  {{ $amenity->name }}</span> 
                           @endforeach   
                           @else
