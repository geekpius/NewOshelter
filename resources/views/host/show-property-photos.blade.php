...
<div class="col-lg-12 img-thumbnail mb-3 propertyImage">
    <i class="fa fa-trash text-danger float-left clearfix remove-property-image btnDelete" data-href="{{ route('host.property.photos.delete',$image->id) }}"> Remove</i>
...
<div class="col-lg-6 col-sm-4 col-xs-4 img-thumbnail mb-2 p-2 propertyImage">
    <i class="fa fa-trash text-danger float-left clearfix remove-property-image btnDelete" data-href="{{ route('host.property.photos.delete',$item->id) }}"> Remove</i>
    <img src="{{ asset('assets/images/properties/'.$item->image) }}" class="property-image-width" heigth="120" alt="property_photo{{$i}}">
    <input type="text" data-id="{{ $item->id }}" name="caption" value="{{ $item->caption }}"  class="form-control inputCaption" placeholder="Caption">
</div>
@endforeach
...
<script>
    ...
    $(".inputCaption").on("focusout", function(e){
        ...
        $.ajax({
            url: "{{ route('host.property.caption.submit') }}",
            type: "POST",
            data: data,
            success: function(resp){
