                                ...
                                <table id="datatable" class="table table-striped small">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><a target="_blank" href="{{ route('property.preview', $auction->property_id) }}">{{ $auction->property->title }}</a></td>
                                            @php $image = (empty($auction->owner->image))? 'user.svg':'users/'.$auction->owner->image; @endphp
                                            <td><img src="{{ asset('assets/images/'.$image) }}" alt="{{ $auction->owner->name }}" class="thumb-sm rounded-circle mr-2">{{ $auction->owner->name }}</td>
                                            <td>
                                                ...
                                                @elseif ($auction->isDoneAttribute())
                                                    <span class="text-success"><i class="fa fa-check-circle"></i> Attend</span>
