        ...
        <div class="pt-4">
            <div class="row">
                <div class="col-sm-12">   
                    ...
                    @else
                        @if (empty(Auth::user()->profile->gender) || empty(Auth::user()->profile->dob) || empty(Auth::user()->profile->marital_status) || empty(Auth::user()->profile->city) || empty(Auth::user()->profile->occupation) || empty(Auth::user()->profile->emergency))
                            <div class="row mb-5">
                                <div class="col-sm-6">
                                    <span><span class="text-muted">Legal Name:</span> {{ Auth::user()->name?? 'N/A' }}</span><br>
                                    <span><span class="text-muted">Gender:</span> {{ Auth::user()->profile->gender?? 'N/A' }}</span><br>
                                    <span><span class="text-muted">Date of Birth:</span> {{ Auth::user()->profile->dob?? 'N/A' }}</span><br>
                                    <span><span class="text-muted">Marital Status:</span> {{ ucwords(Auth::user()->profile->marital_status?? 'N/A') }}</span><br>
                                    <span><span class="text-muted">City:</span> {{ empty(Auth::user()->profile->city)? 'N/A':Auth::user()->profile->city }}</span><br>
                                    <span><span class="text-muted">Profession:</span> {{ Auth::user()->profile->occupation?? 'N/A' }}</span><br>
                                    <span><span class="text-muted">Emergency Contact:</span> {{ Auth::user()->profile->emergency?? 'N/A' }}</span><br>
                                </div>
                                <div class="col-sm-6">
                                    <p class="mt-3">
                                        Seeing this information means you haven't updated your account information. 
                                        <a target="_blank" href="{{ route('account.info') }}">Update your account information here</a>
                                    </p>
                                </div>
                            </div><!-- end row --> 
                        @else
